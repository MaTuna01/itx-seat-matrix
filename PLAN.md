...
| 3 | **좌석맵 조회가 순수 조회인가** — 호출 후 코레일 앱/계정에 임시 홀드·장바구니 흔적이 남지 않는가 | **No → 프로젝트 재검토.** 조회가 예약 프로세스 개시를 수반하면 정차역당 2회 × 구간 N회 호출의 부하·흔적이 완전히 다른 문제가 됨 | ⚠️ **판정 불가** — 좌석맵을 호출할 수단이 없어 순수성 이전 단계에서 막힘. 로그인이 `MACRO ERROR`로 거부됨(암호화 키 발급 `code.do`는 통과, `login.Login`에서 차단) |
| 4 | korail2 응답에 **지연 정보**가 포함되는가 (D-12 검증을 여기로 앞당김) | 있음 → DelayPort 실구현이 공짜. 외부 API 검증 불필요 | ⚠️ **미판정(소스상 있음)** — `ScheduleView` 응답의 `h_expct_dlay_hr`(예상 지연)를 korail2·letskorail 모두 파싱한다. 실측은 조회가 가능해진 뒤에만 |
...
| 6 | 좌석맵 응답이 **전체 좌석+상태**를 주는가, **구매 가능 좌석만** 주는가 (→ D-18) | 전체 → 병합이 단순 조인. 구매 가능만 → **부재 추론 규칙** 적용 (5절 '좌석 유니버스') | ⚠️ **판정 불가(실측)** — 소스상 `ResidualSeatsResearch.do`는 좌석마다 `h_sale_psb_flg`(판매가능 Y/N)를 주고 `h_max_seat_no`·`h_psb_seat_cnt`를 함께 반환하므로 **전체 좌석+상태일 가능성이 높으나** 실측 불가 |
...
- `current_seg_idx = max i s.t. arrival(stops[i]) ≤ now + 지연보정` — **도착시각만 기준**
...
### 코레일 자격증명 — Phase 0 결과에 따라 3갈래 (→ D-14)
| Phase 0 결과 | 처리 |
|---|---|
| 비로그인 조회 가능 | **이 절 이하 전부 삭제.** `korail_id/korail_pw_enc` 컬럼, `PUT /api/me/korail`, Fernet 코레일 암복호화 모두 불필요 |
| 로그인 필수 + 조회용 계정 가능 | **조회 전용 부계정**의 자격증명을 저장. 정기권 본계정과 분리해 세션 충돌(검표 순간 코레일 앱 로그아웃) 리스크 제거 |
| 로그인 필수 + 본계정만 가능 | 동시 세션 정책을 실측 확인 후 진행 여부 재판단 |
- 저장하는 경우: `korail_pw_enc`는 **Fernet 대칭키 암호화**, 키는 env `SECRET_KEY`
