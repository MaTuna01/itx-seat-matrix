...
> - v30: **코레일 조회를 익명 경로로 전환 (D-60, 이슈 #84)** — 2026-09-15 18:11 KST부터 코레일이 **인증된** `ScheduleView`를 HTTP 403으로 막았다(같은 날 07:07까지는 200). 토큰 스킴·앱 버전·IP·기기 지문·TLS 지문·계정을 소거법으로 전부 배제했고, 아이폰 정품 앱은 같은 계정으로 멀쩡했다. 남은 변수는 **요청 모양** 하나 — 로그인·`Key`·`Sid`를 빼고 파라미터를 쿼리스트링에 실은 **익명 경로는 그대로 열려 있고 우리가 쓰는 조회 3종을 전부 커버한다**. D-14/D-22의 "로그인 필수" 전제가 뒤집혔다. 자격증명을 아예 갖지 않게 되어 D-35 실패군과 계정 플래그 위험이 함께 사라진다
>
...
> **v30 개정 (→ D-60)**: 이 항목이 깔아둔 **"로그인 필수"** 전제는 조회 3종에 대해
> **사실이 아니었다.** 2026-09-15 실측으로 로그인 없이 `ScheduleView` →
> `TrainResearch` → `ResidualSeatsResearch`가 전부 통과하는 것이 확인됐다.
> 아래 서술은 당시 판단으로 남긴다 — 문제 1(세션 충돌)은 로그인을 안 하게 되어
> 원인째 사라졌다.
...
> **v30 개정 (→ D-60)**: **우회 의존은 유지되지만 로그인 의존은 사라졌다.**
> `x-dynapath-m-token`은 여전히 필요하다(그래서 PR #54 의존도 그대로다). 다만
> 2026-09-15 코레일이 **인증된** `ScheduleView`를 HTTP 403으로 막으면서, 로그인·
> `Key`·`Sid` 없이 부르는 익명 경로로 전환했다. "로그인 필수 + 본계정"이라는 이
> 항목의 결론은 그 시점부터 무효다.
...
### D-60. 코레일이 **데이터센터 IP를 차단한다** — EC2 배포가 성립하지 않는다 *(v30, 이슈 #84)*

**사건**: 2026-09-15 18:11 KST부터 `/api/trains/search`가 전부 500. 최하단은
`HTTPError: 403 Forbidden ... seatMovie.ScheduleView`. **같은 날 07:07까지는 200이었다**
— 그 사이 우리 쪽 배포·재시작·IP 변경은 없었다.

**결론부터**: 코레일이 그날 낮에 **VPN·데이터센터 IP 차단**을 켰다. EC2(AWS)에서
나가는 모든 코레일 호출이 막힌다. 아래 "최종 확인된 원인"이 서버 응답 원문이다.
**12절이 전제한 EC2 상시 가동 모델이 이 한 줄로 무너진다.**

**진단 (소거법)**. 이 사건의 값어치는 "무엇이 원인인가"보다 **"무엇이 원인이 아닌가"를
어떻게 확정했는가**에 있다. 가설을 하나씩 실측으로 죽였다:

| 가설 | 판정 | 죽인 근거 |
|---|---|---|
| DynaPath 토큰 스킴 회전 | ❌ | 같은 알고리즘·같은 `APP_VERSION=250601002`로 익명 호출 200 |
| 앱 버전 노후 | ❌ | 동일 |
| IP 차단 (AWS/가정) | ❌ | 403 나던 **바로 그 IP**에서 익명 호출 200 |
| 기기 지문 (모델·device_id) | ❌ | 모델·`device_id`를 3종으로 바꿔도 전부 403 |
| TLS/JA3 클라이언트 지문 | ❌ | 같은 `python-requests`로 익명 200 |
| 계정 차단 | ❌ | 아이폰 정품 앱에서 같은 계정 정상. 익명 경로는 계정이 관여조차 안 함 |
| 인증 요청 **모양** (`Key`·`Sid`·파라미터 위치) | ❌ | 2차 분리에서 전부 죽었다 |
| 토큰의 `it`(앱 기동 시각) 노후 | ❌ | 24시간 된 `it`로도 200 |
| **로그인된 세션** | ❌ | **3차에서 죽었다 — 재현 실패** |
| **원인** | **미상** | 아래 "닫지 못한 것" |

**2차 분리 — 1차 결론이 너무 컸다.** 처음에는 "로그인·`Key`·`Sid`를 뺀 요청만
통과한다"로 적었는데, 그건 **변수 4개를 묶어서 말한 것**이었다(① 로그인 쿠키
② `Key` ③ `Sid` ④ 파라미터 위치). 게다가 **Login 자체는 200이다** — Login은 `Sid`를
본문에 싣는 dynapath 경로이므로, `Sid`도 본문 폼도 차단 대상이 아니라는 뜻이었다.
그래서 익명 세션에 변수를 하나씩 되붙여 실측했다 (전부 자격증명 없이 가능하다 —
`Sid`는 알고리즘, `Key`의 로그인 전 기본값은 상수다):

| 실험 | 결과 |
|---|---|
| 익명 + 쿼리스트링 (현 구현) | 200 |
| 익명 + **본문 폼** (옛 구현과 같은 위치) | **200** → ④ 무죄 |
| 익명 + 본문 + **`Sid`** | **200** → ③ 무죄 |
| 익명 + 본문 + **`Key`** | **200** → ② 무죄 |
| 익명 + 본문 + `Sid` + `Key` (= 옛 요청 − 로그인 쿠키) | **200** |

**요청 필드로는 403을 재현할 수 없었다.** 옛 요청과 똑같은 본문을 만들어도, 로그인
쿠키만 없으면 통과한다. 남은 변수는 **로그인된 세션 상태** 하나다.

**3차 — 로그인 세션마저 무죄. 원인은 닫지 못했다.** 2차까지의 결론("로그인한 채로
부르는 것이 표적")을 확인하려고 다음 날 Postman으로 **완전한 재현**을 시도했다:
실계정 로그인 → 그 세션의 쿠키 + 로그인이 준 **진짜 `Key`** + `Sid` + 본문 파라미터.
옛 코드가 보내던 것과 같은 요청이다.

**200이 나왔다. 재현 실패다.** 쿠키를 지운 대조군도 200이었다.

| 시점 | 클라이언트 | 인증 | 결과 |
|---|---|---|---|
| 9/15 07:07 | 우리 파이썬 (EC2) | 로그인 | 200 |
| 9/15 18:11~ | 우리 파이썬 (EC2) | 로그인 | **403** |
| 9/15 저녁 | 우리 파이썬 (맥) | 로그인 | **403** |
| 9/15 저녁 | 우리 파이썬 (맥) | 익명 | 200 |
| 9/16 | **Postman (맥)** | 로그인 | **200** |
| 9/16 | 우리 파이썬 (맥) | 익명 | 200 |

---

## ★ 최종 확인된 원인 (2026-09-15 저녁, EC2에서 익명 호출)

위의 1~4차 추적은 **전부 헛다리였다.** EC2에서 직접 부르자 코레일이 답을 그대로 줬다:

```json
{"code":-8202,"message":"VPN 또는 데이터센터를 통해서는 서비스를 이용할 수 없습니다."}
```

**코레일이 데이터센터 IP를 차단한다.** 인증도, 토큰도, 요청 모양도 무관하다 —
저 응답은 **익명 요청**에 온 것이다.

**왜 그렇게 오래 헤맸나**: 추적 내내 실험을 **맥(가정용 회선)에서** 돌렸다. 차단되지
않은 네트워크에서 아무리 정교하게 소거법을 돌려도 원인에 닿을 수 없다. 확인된 403은
전부 EC2발이었는데, 정작 **EC2에서는 단 한 번도 요청을 보내보지 않았다.** 관측 지점을
장애 지점과 맞추는 것이 가설을 늘리는 것보다 먼저다.

관측이 전부 이 한 줄로 설명된다:

| 관측 | 설명 |
|---|---|
| 아침 07:07 성공 → 18:11 실패, 우리 변경 없음 | 코레일이 그날 낮에 DC 차단을 켰다 |
| 맥·아이폰·Postman 전부 정상 | 가정용 회선이라 차단 대상이 아니다 |
| 익명 경로가 맥에서 항상 200 | 같은 이유 — 경로가 아니라 **회선**이 달랐다 |
| 403 31건이 전부 `ScheduleView` | 그 시간대에 부른 코레일 엔드포인트가 그것뿐 |

**따라서 익명 전환(아래)은 이 장애를 고치지 못한다.** 그 가치는 자격증명을 갖지
않는 것(D-35 실패군 제거, 계정 위험 제거)에 있지, 복구에 있지 않다. **복구하려면
가정용 회선에서 코레일을 호출해야 한다** — 배포 위치의 문제다.

---

### 이하 1~4차 추적 기록 (전부 오답 — 과정으로 남긴다)

**4차 — 빠져 있던 사실이 나타났다.** 재현 실패를 보고한 뒤에야 알게 된 것:
**그날 낮에 `korail_pw`가 빈 채로 로그인 요청을 여러 번 보냈다** (Postman 변수를
비워둔 줄 모르고 테스트 중이었다). 그 시각이 **07:07(마지막 정상)과 18:11(첫 403)
사이의 공백 구간에 정확히 들어간다.** 우리가 "코레일이 낮에 규칙을 바꿨다"고 읽었던
그 창이다.

**가장 잘 설명하는 가설 (확정 아님)**: 반복된 로그인 실패가 코레일의 **계정 단위
비정상 이용 판정**을 불러, 그 계정의 인증된 API 호출이 한동안 403으로 막혔다.
코레일 응답 자체가 경고한다 — `비밀번호 5회 연속 오류시 로그인이 제한됩니다`.

이 가설은 지금까지의 관측을 **전부** 설명한다:

| 관측 | 설명 |
|---|---|
| 우리 쪽 변경이 없는데 낮에 갑자기 깨짐 | 빈 비밀번호 로그인 반복이 그 창에 있었다 |
| EC2와 맥, **다른 IP 둘 다** 403 | IP가 아니라 **계정** 단위 판정이므로 |
| 익명 경로는 내내 멀쩡 | 계정이 관여하지 않으므로 |
| 아이폰 정품 앱은 정상 | 사람 속도의 정상 사용 + 기존 세션 |
| 몇 시간 뒤 스스로 복구 (재현 실패) | 한시적 조치가 풀렸다 |
| 로그인은 통과하는데 `ScheduleView`만 403 | 단순 5회 잠금이 아니라 **이용 판정** 쪽 |

**여전히 미확정**: 마지막 행이 추론이다. 코레일이 어떤 판정을 얼마나 걸었는지는
외부에서 알 수 없다. 실패 횟수와 정확한 시각도 모른다. 확증하려면 코레일이 보냈을
보안 알림을 확인하거나, 옛 경로로 지금 다시 불러 200이 나오는지를 봐야 한다.

**그러나 전환의 근거는 오히려 강해졌다.** 원인이 "코레일이 예매 경로를 조였다"가
아니라 **"우리 도구가 사용자의 실계정을 위험에 빠뜨릴 수 있다"**이기 때문이다.
조회 전용 통근 도구에서, 테스트 실수 하나가 **실제로 기차를 타는 데 쓰는 계정**을
제한시킬 수 있다는 뜻이다. D-14가 걱정했던 "통근 도구가 통근을 방해하는 역설"이
예상과 다른 경로로 실현될 뻔했다. 익명 전환은 그 연결선을 끊는다 — 우리가 가지지
않은 계정은 우리가 망가뜨릴 수 없다.

> **교훈으로 남길 것**: 이 항목은 원인을 두 번 잘못 단정했다가 실험으로 뒤집혔다
> (1차 "인증 요청 모양" → 2차 "로그인 세션" → 3차 둘 다 무죄 → 4차 계정 판정).
> **증상이 사라졌다고 원인을 안 것이 아니다.** 그리고 진짜 원인은 코드나 코레일이
> 아니라 **로그에 남지 않은 사람의 행동**이었다 — 소거법을 아무리 돌려도 관측 밖에
> 있는 변수는 잡히지 않는다. 장애를 볼 때 "그 창에 내가 뭘 했나"를 먼저 물어야 한다.
> 결정 이력에는 확인된 것과 확인되지 않은 것을 갈라 적는다 — 그럴듯한 서사가
> 미확인 사실보다 해롭다.

**결정**: 조회 3종을 익명으로 전환한다. 요청 모양의 차이는 이렇다.

| | 기존 (403) | 익명 (200) | 403의 원인인가 |
|---|---|---|---|
| **로그인 세션 쿠키** | 있음 | **없음** | **★ 이것** |
| `Key` / `Sid` | 보냄 | 안 보냄 | ✗ (무죄, 2차 분리) |
| `ScheduleView` 파라미터 | POST 본문 | 쿼리스트링 (본문 비움) | ✗ (무죄, 2차 분리) |
| 인증 수단 | 로그인 쿠키 | `ScheduleView` 응답의 **익명 쿠키** | — |
| `x-dynapath-m-token` | dynapath 경로만 | **3종 전부** | — |

`Key`·`Sid`·파라미터 위치는 **바꿀 필요가 없었다**(무죄). 그래도 익명 형태로 맞춘 것은
실측으로 200을 확인한 조합이 그것이고, 로그인 산출물을 코드에서 없애는 편이 "실수로
다시 로그인 상태가 되는" 경로를 원천 차단하기 때문이다.

**순서가 곧 인증이다.** 1번이 발급한 쿠키를 2·3번이 물려받는다 — 2번을 먼저 부르면
`P058`. `_seat_map_sync`가 이미 구간마다 `find_train`(=ScheduleView) → `car_list` →
`seat_states` 순으로 부르고 있어 **호출 구조는 바꿀 필요가 없었다.**

**세션 캐시를 없앴다** — 이게 이번 전환에서 유일하게 새로 생긴 위험이었다. 예전엔
자격증명별 클라이언트를 캐시했다(매 조회 로그인은 예절 위반이자 세션 충돌, D-14 문제 1).
로그인이 사라지자 그 이유가 없어졌고, 오히려 **캐시가 해로워졌다**: 익명 쿠키는 순서에
묶여 있어서 인스턴스를 공유하면 병렬 구간 조회(Semaphore 3)에서 구간끼리 쿠키를
덮어쓴다. 그래서 **조회 한 건 = 클라이언트 하나 = 세션 하나**로 바꿨다. 로그인이 없으니
생성도 공짜다.

**호출 예절**: 구간당 `1(ScheduleView) + 1(TrainResearch) + 잔여 있는 일반실 호차 수`로
**D-27과 동일하다.** 로그인 왕복이 빠져 오히려 한 번 줄었다.

**차단 처리도 함께 고쳤다** (같은 사건이 드러낸 두 결함):

1. **403이 500으로 샜다.** 500은 "우리 버그"라는 뜻이라 틀리다. 403을 그대로 내리는
   것도 틀리다 — 그건 "요청한 **사용자**에게 권한이 없다"는 뜻이고, 이 앱은 이미
   관리자 전용 기능(D-24)에 403을 쓰고 있어 화면이 둘을 구분하지 못하게 된다.
   상위 서버가 준 오류이므로 **502**가 맞다. 4xx만 차단으로 보고 5xx는 일시 장애로
   남겨 재시도 기회를 지킨다.
2. **`KorailBlocked`가 조용히 재시도를 탔다.** 타입이 `RuntimeError` 계열이라
   `_with_retry`의 `ValueError` 제외에 걸리지 않았다 — 자기 docstring은 "재시도 대상이
   아니다"라고 적어두고 실제로는 재시도하고 있었다. 그래서 9/15에 화면 한 번에 코레일을
   두 번씩 두드렸다. 차단 상태의 재시도는 상황을 악화시킬 뿐이다(CLAUDE.md 10).
   이제 재시도 제외 목록에 있다.

**로그**: 거부 시 상태코드·`server` 헤더·본문 앞 300자를 WARNING으로 남긴다. 9/15에는
`403`이라는 사실만 남고 본문이 버려져서, 원인을 가리는 데 **실측 호출이 따로 필요했다.**
사용자 응답에는 사유를 노출하지 않는다.

**얻은 것**: 자격증명을 **아예 보관하지 않게 된다** → `korail_pw_enc`·Fernet·D-35
(`SECRET_KEY` 지문 불일치) 실패군이 통째로 사라진다. 사용자 실계정이 매크로로 플래그될
위험도, 검표 중 로그아웃 위험(D-14 문제 1)도 원인째 없어진다. 무엇보다 **예매 경로를
전혀 건드리지 않는다** — 조회 전용 도구에 맞는 자세다.

**범위**: 이번(1단계)은 어댑터 전환까지다. `cred`는 Port 시그니처에 남기고 쓰지 않으며
`CredentialsRequired`와 409 매핑도 남겼다. **자격증명 저장 자체를 걷어내는 일(DB 컬럼,
설정 UI, Fernet)은 2단계로 분리한다** — API·DB·프론트로 번져 복구가 늦어진다.

**남은 취약성 (정직하게)**: 익명 경로도 코레일이 언제든 조일 수 있다. 취약성의 *종류*는
D-22와 같고, 줄어든 것은 **노출 면적**(예매 경로를 안 건드림)과 **피해 규모**(계정이
걸려 있지 않음)다. 깨지면 증상은 두 가지다 — `MACRO ERROR`(앱 로직까지 닿음) 또는
**HTTP 403**(게이트웨이에서 잘림). 둘 다 `KorailBlocked`로 모인다.

**따로 기록해 둘 것 (이번 범위 밖, 기존 동작)**: `find_train`은 `at`을 넘기지 않아
**자정부터** 검색하는데 코레일은 10건만 돌려준다. 늦은 시간대 열차는 그 10건에 없을 수
있고, 그러면 "구간에 열차 없음 → 전 좌석 판매"로 읽힌다(D-36 경로). 이번 403과 무관한
선재 동작이라 건드리지 않았으나 **조용히 틀릴 수 있는 자리**다.
