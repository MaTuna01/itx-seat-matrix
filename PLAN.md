...
- `clear_all` 좌석이 없으면 `clear_until` 최댓값 좌석 제시 ("최소 안양까지는 확보")
  - **단, 실효 시작 구간이 이미 팔린 좌석은 추천 대상이 아니다** (`clear_until == start`).
    즉 현재 추천은 **"지금 당장 앉을 수 있는 좌석"만** 대상이다. "지금은 못 앉지만 몇
    정거장 뒤부터 앉을 수 있는 좌석"의 추천은 **미정**이다 (→ D-45)
...
- 프리셋이 있으면 1~2단계를 건너뛴다 (자주 쓰는 구간 = 행 추가, 원칙 1·3).
  **아직 프론트가 없다** — 구간이 둘 이상 필요해지면 만든다 (Phase 4, → D-37)
...
| 피그마 화면 | 현재 파일 |
|---|---|
| 01 로그인 | `web/src/Login.jsx` |
| 02 탑승 등록 / 04 열차 선택 | `web/src/Setup.jsx` |
| 03 역 검색 (모달 시트) | `web/src/StationPicker.jsx` |
| 05 지금 상태 / 06·07 매트릭스 / 08 오프라인 / 09 조회 실패 | `web/src/SeatMatrix.jsx` |
| 10·11 설정 | `web/src/Settings.jsx` |
...
**배포 시 `web/public/sw.js`의 `CACHE` 버전을 올린다** (`itx-shell-v1` → `v2`). 올리지 않으면
...
일치하며**(입석/착석 양쪽 + 내 좌석 부재 규칙), `uv run pytest`와 프론트 빌드가 통과한다.
...
├── .env.example
├── .github/
│   └── workflows/ci.yml      # 테스트 / arm64 빌드+스모크 / 시크릿 검사 (→ D-44). 배포는 없다
├── scripts/
...
- **`05 지금 상태 (바텀시트)`가 새 데이터를 요구하는지.** 기존 `/matrix` 응답의 재배치라면
  범위 안이고, 아니면 결정 ②에 걸린다.
- **iOS 전용 조정을 어디까지 허용할지.** 접근 경로가 tailnet 전용이라 사실상 아이폰+맥
  브라우저만 쓰지만, 393pt 고정 레이아웃으로 가면 맥 브라우저에서 확인하기 나빠진다.
  개발·디버깅을 맥 브라우저로 해왔다는 점(Phase 1~4 내내)을 고려해야 한다.
