...
| 인증 요청 **모양** | ❌ | 아래 2차 분리에서 전부 죽었다 |
| **로그인된 세션으로 부르는 것** | ✅ | 마지막까지 남은 단 하나의 변수 |
...
트레이스백도 이와 맞는다 — `schedule_view → _post`에서 시작하므로 **로그인은
통과했다.** 코레일은 로그인을 막지 않는다. 막는 것은 **로그인한 채로 부르는
`ScheduleView`**, 즉 **예매 진입 경로**다. 매크로 대응의 1순위가 정확히 거기다.
조회 전용 익명 경로는 열어두었다.

> **아직 안 가른 것**: 로그인 세션 안에서도 (a) 쿠키 자체가 표적인지, (b) 로그인
> 응답이 주는 **진짜 `Key`** 값이 표적인지는 분리하지 못했다. 위 실험의 `Key`는
> 로그인 전 기본 상수라 진짜 값이 아니다. 가르려면 실계정 로그인이 필요하다.
> **우리 코드는 둘 다 보내지 않으므로 전환의 타당성에는 영향이 없다.**
