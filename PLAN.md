...
| **인증 요청 모양** | ✅ | 로그인·`Key`·`Sid`를 뺀 요청만 통과 |

트레이스백이 `schedule_view → _post`에서 시작하므로 **로그인 자체는 통과했다.**
표적은 인증된 `ScheduleView`, 즉 **예매 진입 경로**다 — 매크로 대응의 1순위가 거기다.
코레일은 그 경로를 조이고 조회 전용 익명 경로는 열어두었다.
...
| 로그인 | 필수 | **없음** |
| `Key` / `Sid` | 보냄 | 안 보냄 |
| `ScheduleView` 파라미터 | POST 본문 | **쿼리스트링** (본문 비움) |
