...
- `KorailPort`의 korail2 어댑터는 **korail2 PR #54(`dhfhfk:bypassDynapath`) 기반**으로 구현한다
  (→ D-22). PR이 언스트림 머지되지 않았으므로 `git+https://github.com/dhfhfk/korail2.git@bypassDynapath`
  형태의 브랜치 의존이 아니라, 커밋 해시 고정 또는 필요한 DynaPath 토큰 로직만 벤더링하는 것을
  Phase 1 구현 시점에 재확인한다 (브랜치가 rebase/force-push되면 재현 불가 리스크)
...
- **결정**:
  ...
  4. **의존성 고정 방식은 Phase 1 구현 시점에 재확인.** 현재처럼 `git+...@bypassDynapath`
     브랜치를 직접 참조하면 원본 저장소가 브랜치를 rebase/삭제할 경우 재현이 깨진다 —
     커밋 해시 고정 또는 DynaPath 토큰 로직만 별도 벤더링하는 방안을 저울질한다.
  5. **다음 앱 업데이트로 토큰 스킴이 다시 바뀌면 이 어댑터는 깨진다** — 원칙 2(Port 격리)
     덕에 파급 범위는 어댑터 교체로 국한되지만, 유지보수 부채로 남는다는 점은 명시해둔다.
