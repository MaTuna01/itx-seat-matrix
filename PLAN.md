...
접속 기기에 따라 한 번들 안에서 스킨을 고른다. **아이폰/아이패드 → `ios`, 그 외(맥 포함) →
`web`.** `?ui=ios` / `?ui=web` / `?ui=auto`로 강제하며 그 값은 `localStorage`에 고정된다.
...
**진행**: `01 로그인` 완료 (이슈 #25). 나머지는 `skins/web`으로 폴백 중이다.
...
D-50에서 `v4`, Phase 5 `01 로그인`에서 `v5`까지 올라갔다 — **화면을 켤 때마다 올린다**).
올리지 않으면
...
**결정 ④: 맥은 기본으로 web을 받는다.** UA 기준 아이폰/아이패드 → `ios`, 그 외 → `web`.
iPadOS는 맥 UA로 위장하므로 `maxTouchPoints`로 가른다. `?ui=`로 강제하고 `localStorage`에
고정한다. **이것이 D-43 미정 ③의 답이다** — "393pt 고정으로 가면 맥 브라우저에서 확인하기
나빠진다"는 걱정은 web 스킨을 맥의 기본값으로 남기는 것으로 닫힌다.
