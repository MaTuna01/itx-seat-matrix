...
> - v21: **매트릭스는 좁히지 말고 밀어라 (D-51)** — 구간 열 너비를 균등하게 고정하고, 다 안 들어가면 글자를 줄이는 대신 가로로 스크롤한다(좌석 열은 고정). 좌석 액션 바는 문서 끝이 아니라 화면 하단에 고정한다
>
...
- **프로토타입 완성본: `seat-matrix.jsx`** (레포에 포함) — 매트릭스 화면 확정
  ...
  - **구간 열은 너비가 모두 같다. 다 안 들어가면 줄이지 말고 가로로 스크롤한다** (→ D-51).
...
**실기기 확인에서 매트릭스 레이아웃 2건이 나왔고 고쳤다** (이슈 #48 → D-51):
구간 열 정렬, 좌석 선택 액션 바 위치. 나머지 화면은 이상 없었다.
...
### D-51. 매트릭스는 좁히지 말고 밀어라 — 구간 열 균등 + 가로 스크롤 *(v21, 이슈 #48)*

**맥락**: Phase 5를 배포하고 실기기로 확인하다 매트릭스에서 두 가지가 나왔다. 나머지 아홉
화면은 이상이 없었다 — **두 건 다 목업(정차역 5개)에서는 드러나지 않는 것**이었다.
...
- **노선 진행바는 정차역이 여덟을 넘으면 이름을 고른다** — 출발 · 하차 · 지금 향하는 역만.
  12개 노선은 역당 30pt라 이름이 서로 겹친다. 점은 전부 남으니 어디까지 왔는지는 그대로
  읽히고, 구간별 상세는 바로 아래 매트릭스가 말한다. 일곱까지는 역당 51pt라 손대지 않는다.

**함께 한 것**: 프리뷰 하네스에 `?state=long`을 더했다. 기본 픽스처가 5개역이라 **세 증상 다
헤드리스에서 재현되지 않았다** — 목업이 짧으면 목업만 통과한다. 그래서 픽스처를 **실기기에서
깨진 그 노선 그대로**(창원→수원 12개역, `창원중앙` 포함) 맞춰 놨다.
