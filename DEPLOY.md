...
**★ 배포는 가동 시간대 안에서만 된다** (→ D-54). 인스턴스가 평일 06:00~24:00만 깨어 있으므로,
꺼져 있는 동안 `main`에 푸시하면 CD가 **`korail-matrix 를 tailnet에서 찾지 못했다`로 실패한다** —
tailnet에서 피어가 사라진 상태다. **서버를 건드리기 전 단계라 손상은 없고**, 다음 기동 뒤
Actions에서 **Re-run**하면 그대로 배포된다.

급하면 AWS 콘솔(모바일 앱 포함)에서 인스턴스를 시작한 뒤 Re-run해라. 1~2분이면 tailnet에
다시 붙는다. 다만 **그렇게 켠 인스턴스는 다음 23:50까지 켜져 있다** — 정지 판단은 그때
한 번만 하므로, 새벽 3시에 켜서 배포하면 그날은 21시간을 켜둔 셈이 된다.
...
### 정차역 캐시 재적재 (D-29 → D-58 자동화)

정차역 캐시는 "최근 실적" 템플릿이라 시각표 개정으로 열차번호가 바뀌면 낡는다.
캐시에 없는 열차번호는 조용히 틀리지 않고 사용자용 404 문구로 드러난다 (이슈 #75).

**평상시는 스케줄러가 매일 06:05·12:05 KST에 자동 재적재한다** (D-58, 이슈 #76).
게이트가 있어 이미 D-1 실적을 반영했으면 스킵 — 첫 성공만 네트워크를 쓴다.
개편 첫날은 실적이 하루 늦게 공개돼 다음 날 새벽에 자동 회복된다 (통근 07:1x 이전).
확인:

```bash
docker compose logs app | grep "정차역 캐시 재적재"
```

수동 재적재는 자동화 실패 로그를 봤거나 다음 자동 실행을 못 기다릴 때 쓴다:
